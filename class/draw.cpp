...
bool draw::line(Mat* img, Point start, Point end, Scalar color, int thickness,
                int lineType) {
...
bool draw::rectangle(Mat* img, Point pIni, Point pFim, unsigned int borda,
                     Scalar borderColor) {
...
bool draw::rectangle(Mat* img, Rect regiao, unsigned int borda,
                     Scalar borderColor) {
  cv::rectangle(*img, regiao, borderColor, (int)borda, 0);
}
...
bool draw::triangle(Mat* img, Point p1, Point p2, Point p3, Scalar color,
...
bool draw::circle(Mat* img, Point center, int radius, Scalar color,
                  int thickness, int lineType, int shift) {
  cv::circle(*img, center, radius, color, thickness, lineType, shift);
}

bool draw::text(string text, Mat* img, Point posicao, Scalar color,
                double fontScale, int fontFace, int thickness, bool box,
                Scalar boxColor) {
  if (box) {
    int baseline = 0;
    Size textSize =
    ...
    Point ptBox1(posicao.x - thickness, posicao.y - thickness),
        ptBox2(posicao.x + textSize.width + thickness,
               posicao.y + textSize.height + thickness);
...
bool draw::textCenter(string text, Mat* img, Point center, Scalar color,
                      double fontScale, int fontFace, int thickness, bool box,
                      Scalar boxColor) {
  int baseline = 0;
  ...
  draw::text(text, img, textPos, color, fontScale, fontFace, thickness, box,
             boxColor);
}
...
void dimnova(unsigned int* width, unsigned int* height, unsigned int dimMax) {
  if (*width > *height) {
    *height =
        (unsigned int)((float)dimMax * ((float)(*height) / (float)(*width)) +
                       0.5);
    *width = dimMax;
  } else {
    *width =
        (unsigned int)((float)dimMax * ((float)(*width) / (float)(*height)) +
                       0.5);
    *height = dimMax;
  }
}
...
void draw::redim(Mat* img, double factor) {
  ...
  if (width < img->cols && height < img->rows) redim(img, width, height);
}
