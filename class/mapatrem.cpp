...
MapaTrem::~MapaTrem() {
  for (Path* traj : trajetos) delete traj;
...
bool MapaTrem::Sensores(Point3f p1, Point3f p2, bool& a1, bool& a2, bool& b1,
                        bool& b2, bool& c) {
...
bool MapaTrem::Trem1Pos(float param, int trajeto) {
  ...
  if (trajeto < 0 || trajeto >= trajSize) return false;
...
bool MapaTrem::imgAdd(cv::Mat& fundo, cv::Mat& add, int x, int y,
                      Vec3b transpColor) {
  Vec3b* pixel;
  Rect local;
  ...
  if (local.x < 0 || local.y < 0 || fundo.cols < local.x + local.width ||
...
bool MapaTrem::Exec() {
  while (1) {
    ...
    msg = t1msg;
    sMapa.Unlock();
    imgAdd(img, trem1, (int)(pt.x + 0.5), (int)(pt.y + 0.5),
           Vec3b(255, 255, 255));

    if (msg != "") draw::text(msg, &img, Point(10, 50), Scalar::all(0), 1);
    ...
    msg = t2msg;
    sMapa.Unlock();
    imgAdd(img, trem2, (int)(pt.x + 0.5), (int)(pt.y + 0.5),
           Vec3b(255, 255, 255));

    if (msg != "") draw::text(msg, &img, Point(10, 450), Scalar::all(0), 1);
