...
class linep {
 protected:
  Point3f start;
  Point3f velocity;
  Point3f velocityN, velocityNxy,
      velocityNxz;  // normalized in meters or units metrics it uses --
                    // parameter is the distance to start

  // float AngleOfTreePointsXZ(Point3f &origin, Point3f &direction1, Point3f
  // &direction2);
 public:
  ...
  bool PointsFromDistance2ParamXZ(float parameterPoint, float dist,
                                  Point3f *pLeft, Point3f *pRight);
  bool PointsFromDistance2StartXZ(float dist, Point3f *pLeft, Point3f *pRight);
  bool PointsFromDistance2ParamXY(float parameterPoint, float dist,
                                  Point3f *pLeft, Point3f *pRight);
